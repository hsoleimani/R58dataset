...
############################################################### 
# Reduce vocabulary size. 2 options:
# 1) remove words with <= 3 occurrences (first block -- commented out)
N = 0
vocabs_reduced = {}
import operator
sorted_dic = sorted(vocabs.items(), key=operator.itemgetter(1)) # sort by wrd index
for vpair in sorted_dic:
	w = vpair[0]
	n_old = vpair[1]
...
# Or 2) take the top 2000 words:
'''N = 0
vocabs_reduced = {}
top_wrd_indices = np.argsort(-vocabcnts)[:2000]
import operator
sorted_dic = sorted(vocabs.items(), key=operator.itemgetter(1)) # sort by wrd index
for vpair in sorted_dic:
	w = vpair[0]
	n_old = vpair[1]
	if n_old in top_wrd_indices:
		vocabs_reduced.update({w:N})
		N += 1 
vocabs = vocabs_reduced.copy()'''
############################################################### 
...
for d in train_ind:
	...
	ld = len(doc_wrds)
	if ld > 5:
		doc_txt = str(ld) + ' ' + ' '.join(['%d:%d' %(w,doc_cnts[i]) for i,w in enumerate(doc_wrds)])
	else:
		continue
	fptr.write(doc_txt+'\n')
